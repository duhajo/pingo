class TagsController < ApplicationController
  ...
  def show
    ...
    @supplies = Job.tagged_with(@tag.name).where(:type => 1).joins(:user).select("jobs.*, users.email as user_email, users.name as user_name, users.id as user_id")
    @demands = Job.tagged_with(@tag.name).where(:type => 2).joins(:user).select("jobs.*, users.email as user_email, users.name as user_name, users.id as user_id")
